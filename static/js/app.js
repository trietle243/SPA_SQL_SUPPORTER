...
function handleObjectSelection(e) {
    if (e.target.value) {
        ...
        showWorkingBanner(document.getElementById('objectSelect'), objectName);
    }
}
...
async function handleConfirm() {
    ...
    // 2. DB Insert Action
    try {
        // Prefer to parse version from the generated filename (if present).
        let versionToLog = null;
        const vm = /^V(\d+)_(\d+)__/i.exec(currentFileName || '');
        if (vm) {
            versionToLog = `V${vm[1]}_${vm[2]}`;
        }
